...
# Find drawEquipment() {
match = re.search(r'drawEquipment\(\)\s*\{', text)
if match:
    # Now find the first abyssal_summoner *after* this match
    sub_text = text[match.end():]
    target = "    if (this.type === 'abyssal_summoner') {"
    idx = sub_text.find(target)
    if idx != -1:
        insert_pos = match.end() + idx
        text = text[:insert_pos] + draw_block + text[insert_pos:]
...
with open(r'c:\Users\Rexshimura\Desktop\[JULY]-SIDE-PROJECTS\unit_battle_simulator\js\entities\Unit.js', 'w', encoding='utf-8') as f:
    f.write(text)
print('Fixed Restrictor draw logic location perfectly.')
