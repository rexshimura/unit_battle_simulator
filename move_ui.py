...
"""
if old_cd in text:
    text = text.replace(old_cd, '')
else:
    print('Old CD not found exactly, maybe whitespace diff. Trying regex.')
    text = re.sub(r'if \(this\.type === \'restrictor\'\) \{\s*const PUSH_COOLDOWN = 5000;[\s\S]*?\}\s*\}', '', text, count=1)

match = re.search(r'uiElements\.ctx\.fillText\([^;]+;\s*\}', text)
if match:
